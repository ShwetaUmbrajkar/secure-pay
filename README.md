...
git clone <this-repo>
cd secure-pay
cp .env.example .env   # then edit JWT_SECRET; GROQ_API_KEY is optional
...
First run will take a few minutes (Maven dependency download, npm install, Kafka image pulls).

### Option B — Run services individually
```bash
# Postgres + Kafka only, via Docker
...
- **Maven can't resolve a dependency version** — this repo's code was written without a live Maven Central connection to compile against; if `mvn clean install` fails on a specific artifact version, bump that dependency's version in `pom.xml` (Spring Boot 3.2.x line is a safe target) and retry.
- **Kafka connection refused from the backend** — Kafka can take 10-20s longer to be ready than Postgres on first boot; re-run `docker compose up backend` once Kafka's logs show it's accepting connections.
- **`data.sql` seed doesn't apply** — confirm `spring.sql.init.mode=always` and `spring.jpa.defer-datasource-initialization=true` are present in `application.yml` (they are, by default, in this repo).

---

## 10. API Examples / Screenshots

No screenshots included in this repo yet (it hasn't been run against a live UI). Once you have it running, here's the demo flow to capture for your portfolio/interview:

1. Register → Login → land on Dashboard with `ACC-001` (seeded, ₹50,000 balance)
2. Make a small transaction (e.g. ₹500) → resolves instantly to `LOW_RISK`
3. Make a large transaction (e.g. ₹150,000) → shows `PENDING`, then auto-updates to `HIGH_RISK` or `LOW_RISK` within a couple seconds as the agent investigates
4. Open the transaction details page and refresh — note the risk badge and (if you want to extend it) surface the agent's `summary` text there too
5. Log in as an admin user (manually set `role = 'ADMIN'` for a user row, or add an admin-promotion endpoint) → view `/admin` fraud-monitoring dashboard

---

## 11. Testing

- **Backend:** `cd backend && mvn test` — JUnit 5 + Mockito tests for `TransactionService` (balance updates, insufficient-balance rejection, fraud-event triggering threshold logic).
- **AI Agent:** `cd ai-agent && pytest -v` — tests for the risk engine's scoring behavior, the rules engine's KYC/large-transaction triggers, and the agent's end-to-end tool-calling trace.
- **CI:** GitHub Actions (`.github/workflows/ci.yml`) runs backend tests, AI agent tests, a frontend build, and a Docker image build for all three services on every push/PR to `main`.
